      ...
      <div class="row align-items-center">
        <div class="col-md-6">
          <h3 class="page-title">
            ...
            Gestion des structures contrôlées
          </h3>
          <p class="text-muted mb-0">Gérez les structures sous contrôle</p>
        </div>
        <div class="col-md-6 text-end">
          <div class="stats-badge">
            <span class="badge bg-primary me-2">
              <i class="bi bi-database"></i> Total: {{ structureControllerStore.getterStructureController.length }}
            </span>
          </div>
        </div>
      </div>
    ...
    <div class="table-wrapper">
      <div class="table-responsive">
        <table class="table modern-table">
          ...
          <tbody v-if="filteredStructureController.length > 0">
            <tr v-for="(item, index) in filteredStructureController" :key="item.id" class="hover-effect">
              <td class="text-center">
                <span class="row-number">{{ index + 1 }}</span>
              </td>
              <td>
                <div class="structure-info">
                  <span class="structure-code">{{ item.code }}</span>
                  <span class="structure-libelle">{{ item.libelle }}</span>
                </div>
              </td>
              <td class="text-center">
          ...
          <div class="modal-header">
            <div class="modal-header-content">
              ...
              <div>
                ...
                <p class="modal-subtitle">Ajoutez une nouvelle structure contrôlée</p>
              </div>
            </div>
            ...
            <form class="needs-validation">
              ...
              <div class="form-group mb-3">
                ...
                <input
                  ...
                  type="text"
                  placeholder="Ex: Direction financière, Service comptable, etc."
          ...
          <div class="modal-header">
            <div class="modal-header-content">
              ...
              <div>
                ...
                <p class="modal-subtitle">Modifiez les informations de la structure</p>
              </div>
            </div>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
...
const form = reactive<StructureController>({
  id: 0,
  code: '',
  libelle: '',
})

const formEdit = reactive<StructureController>({
  id: 0,
  code: '',
  libelle: '',
})
...
const generatedCode = computed(() => {
  if (form.libelle) {
    ...
    const existingCodes = structureControllerStore.getterStructureController.map(s => s.code)
    let counter = 1
    let uniqueCode = base
...
const filteredStructureController = computed(() => {
  if (!searchTerm.value) return structureControllerStore.getterStructureController

  const keyword = searchTerm.value.toLowerCase()
  return structureControllerStore.getterStructureController.filter(item =>
...
function AjouterFonctionLocal() {
  ...
  if (!$v.value.$invalid) {
    try {
      ...
      structureControllerStore.AjouterStructureController(objet).then(() => {
...
function ModifierFonctionLocal() {
  ...
  if (!$v_edit.value.$invalid) {
    try {
      isLoading.value = true
      structureControllerStore.modifierStructure(formEdit).then(() => {
        isLoading.value = false
        closeModal()
        annulerChampEdit()
        Swal.fire({
...
const showEditModal = (id: number) => {
  const data = structureControllerStore.getterStructureController.find((item) => item.id === id)
...
function supprimer(id: number) {
  Swal.fire({
    ...
    text: 'Êtes-vous sûr de vouloir supprimer cette structure ?',
    icon: 'warning',
    ...
    if (res.isConfirmed) {
      structureControllerStore.supprimerStructure(id)
      Swal.fire({
        title: 'Supprimé!',
