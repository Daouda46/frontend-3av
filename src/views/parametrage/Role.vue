    ...
    <div class="page-header mb-4">
      <div class="row align-items-center">
        <div class="col-md-6">
          ...
          <p class="text-muted mb-0">Gérez les différents rôles et permissions</p>
        </div>
        <div class="col-md-6 text-end">
          <div class="stats-badge">
            <span class="badge bg-primary me-2">
              <i class="bi bi-database"></i> Total: {{ roleStore.getterRole.length }}
            </span>
          </div>
        </div>
      </div>
    ...
    <div class="table-wrapper">
      <div class="table-responsive">
        <table class="table modern-table">
          ...
          <tbody v-if="filteredRoles.length > 0">
            <tr v-for="(item, index) in filteredRoles" :key="item.id" class="hover-effect">
          ...
          <div class="modal-header">
            <div class="modal-header-content">
              ...
              <div>
                ...
                <p class="modal-subtitle">Ajoutez un nouveau rôle dans le système</p>
              </div>
            </div>
            ...
            <form class="needs-validation">
              ...
              <div class="form-group mb-3">
                ...
                <input
                  ...
                  @input="generateCodeFromLibelle"
                />
                <div class="invalid-feedback" v-if="$v.libelle.$error">
                  Ce champ est requis
                </div>
                <small class="text-muted">Le code sera généré automatiquement à partir du libellé</small>
              </div>
            </form>
          </div>
          ...
          <div class="modal-header">
            <div class="modal-header-content">
              ...
              <div>
                ...
                <p class="modal-subtitle">Modifiez les informations du rôle</p>
              </div>
            </div>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
            ...
            <form class="needs-validation">
              ...
              <div class="form-group mb-3">
                ...
                <div class="code-display">
                  ...
                  <span class="code-hint">Code unique du rôle</span>
                </div>
              </div>
...
const generateCodeFromLibelle = () => {
  if (form.libelle) {
    const words = form.libelle.trim().split(' ')
    let code = ''
    ...
    const existingCodes = roleStore.getterRole.map(r => r.code)
    let counter = 1
    let uniqueCode = code
...
const filteredRoles = computed(() => {
  if (!searchTerm.value) return roleStore.getterRole

  const keyword = searchTerm.value.toLowerCase()
  return roleStore.getterRole.filter(item =>
...
function AjouterFonctionLocal() {
  ...
  if (!$v.value.$invalid) {
    try {
      ...
      roleStore.AjouterRole(objet).then(() => {
...
function ModifierFonctionLocal() {
  ...
  if (!$v_edit.value.$invalid) {
    try {
      isLoading.value = true
      roleStore.modifierRole(formEdit).then(() => {
        isLoading.value = false
        closeModal()
        annulerChampEdit()
        Swal.fire({
...
const showEditModal = (id: number) => {
  const data = roleStore.getterRole.find((item) => item.id === id)
