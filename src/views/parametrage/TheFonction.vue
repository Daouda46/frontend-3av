    ...
    <div class="page-header mb-4">
      <div class="row align-items-center">
        <div class="col-md-6">
          ...
          <p class="text-muted mb-0">Gérez les différentes fonctions de l'organisation</p>
        </div>
        <div class="col-md-6 text-end">
          <div class="stats-badge">
            <span class="badge bg-primary me-2">
              <i class="bi bi-database"></i> Total: {{ fonctionStore.getterFonction.length }}
            </span>
          </div>
        </div>
      </div>
    ...
    <div class="table-wrapper">
      <div class="table-responsive">
        <table class="table modern-table">
          ...
          <tbody v-if="filteredFonctions.length > 0">
            <tr v-for="(item, index) in filteredFonctions" :key="item.id" class="hover-effect">
          ...
          <div class="modal-header">
            <div class="modal-header-content">
              ...
              <div>
                ...
                <p class="modal-subtitle">Ajoutez une nouvelle fonction</p>
              </div>
            </div>
            ...
            <form class="needs-validation">
              ...
              <div class="form-group mb-3">
                ...
                <input
                  ...
                  type="text"
                  placeholder="Ex: Directeur financier, Comptable, etc."
          ...
          <div class="modal-header">
            <div class="modal-header-content">
              ...
              <div>
                ...
                <p class="modal-subtitle">Modifiez les informations de la fonction</p>
              </div>
            </div>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
...
const generatedCode = computed(() => {
  if (form.libelle) {
    ...
    const existingCodes = fonctionStore.getterFonction.map(f => f.code)
    let counter = 1
    let uniqueCode = base
...
const filteredFonctions = computed(() => {
  if (!searchTerm.value) return fonctionStore.getterFonction

  const keyword = searchTerm.value.toLowerCase()
  return fonctionStore.getterFonction.filter(item =>
...
function AjouterFonctionLocal() {
  ...
  if (!$v.value.$invalid) {
    try {
      ...
      fonctionStore.AjouterFonction(objet).then(() => {
        isLoading.value = false
        closeModal()
        annulerChamp()
        Swal.fire({
          title: 'Succès!',
          text: 'La fonction a été ajoutée avec succès.',
          icon: 'success',
          timer: 2000,
  ...
  if (!$v_edit.value.$invalid) {
    try {
      ...
      fonctionStore.modifierFonction(formEdit).then(() => {
        ...
        Swal.fire({
          title: 'Succès!',
          text: 'La fonction a été modifiée avec succès.',
          icon: 'success',
          timer: 2000,
          showConfirmButton: false
        })
...
function supprimer(id: number) {
  Swal.fire({
    ...
    text: 'Êtes-vous sûr de vouloir supprimer cette fonction ?',
    icon: 'warning',
    ...
    if (res.isConfirmed) {
      ...
      Swal.fire({
        title: 'Supprimé!',
        text: 'La fonction a été supprimée.',
        icon: 'success',
        timer: 2000,
        showConfirmButton: false
      })
...
onMounted(async () => {
  try {
    isLoading.value = true
    await fonctionStore.getFonction()
  } catch (error) {
