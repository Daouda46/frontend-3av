<template>
  <div class="exercice-page">
    ...
    <div class="page-header mb-4">
      <div class="row align-items-center">
        <div class="col-md-6">
          ...
          <p class="text-muted mb-0">Gérez les exercices budgétaires de la l'amical</p>
        </div>
        <div class="col-md-6 text-end">
          <div class="stats-badge">
            <span class="badge bg-primary me-2">
              <i class="bi bi-database"></i> Total: {{ exerciceStore.getterExerciceBudgetaire.length }}
            </span>
            <span class="badge bg-success">
              <i class="bi bi-check-circle"></i> Actif: {{ exerciceStore.getterExerciceBudgetaire.filter(e => e.encours == 1).length }}
            </span>
          </div>
        </div>
      </div>
    ...
    <!-- Tableau des années budgétaires -->
    <div class="table-wrapper">
      <div class="table-responsive">
        <table class="table modern-table">
          ...
          <tbody v-if="filteredexerciceBug.length > 0">
            <tr v-for="(item, index) in filteredexerciceBug" :key="item.id" class="hover-effect">
          ...
          <tbody v-else>
            <tr>
              <td colspan="6" class="empty-state">
                ...
                <p>Aucune année budgétaire trouvée</p>
              </td>
            </tr>
          </tbody>
        </table>
        ...
        <div class="modal-content">
          <div class="modal-header">
            <div class="modal-header-content">
              ...
              <div>
                <h5 class="modal-title">Nouvel exercice budgétaire</h5>
                <p class="modal-subtitle">Ajoutez une nouvelle année budgétaire</p>
              </div>
            </div>
          ...
          <div class="modal-body">
            <form class="needs-validation">
              ...
              <div class="form-group mb-3">
                ...
                <input
                  class="form-control"
                  :class="{ 'is-invalid': $v.annee.$error }"
                  type="number"
                  placeholder="Ex: 2024"
                  v-model="form.annee"
                />
                <div class="invalid-feedback" v-if="$v.annee.$error">
                  <span v-if="$v.annee.required.$invalid">Ce champ est requis</span>
                  <span v-else-if="$v.annee.minLength.$invalid">L'année doit avoir 4 chiffres</span>
                  <span v-else-if="$v.annee.maxLength.$invalid">L'année doit avoir 4 chiffres</span>
                </div>
                <div class="form-error" v-if="verif_annee(form.annee)">
          ...
          <div class="modal-header">
            <div class="modal-header-content">
              ...
              <div>
                ...
                <p class="modal-subtitle">Modifiez les informations de l'année budgétaire</p>
              </div>
            </div>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
          <div class="modal-body">
            <form class="needs-validation">
              ...
              <div class="form-group mb-3">
                ...
                <input
                  class="form-control"
                  :class="{ 'is-invalid': $v_edit.annee.$error }"
                  type="number"
                  placeholder="Ex: 2024"
                  v-model="formData.annee"
                />
                <div class="invalid-feedback" v-if="$v_edit.annee.$error">
                  <span v-if="$v_edit.annee.required.$invalid">Ce champ est requis</span>
                  <span v-else-if="$v_edit.annee.minLength.$invalid">L'année doit avoir 4 chiffres</span>
                  <span v-else-if="$v_edit.annee.maxLength.$invalid">L'année doit avoir 4 chiffres</span>
                </div>
              </div>
...
const filteredexerciceBug = computed(() => {
  if (searchTerm.value !== '') {
    return exerciceStore.getterExerciceBudgetaire.filter((ExerciceBud) =>
      ExerciceBud.annee.toString().toLowerCase().includes(searchTerm.value.toLowerCase())
    )
  } else {
    return exerciceStore.getterExerciceBudgetaire
  }
})
...
function supprimer(id: any) {
  Swal.fire({
    ...
    text: 'Êtes-vous sûr de vouloir supprimer cette année budgétaire ?',
    icon: 'warning',
    ...
    if (res.isConfirmed) {
      exerciceStore.supprimerExerciceBudgetaire(id)
      Swal.fire({
        title: 'Supprimé!',
        text: 'L\'année budgétaire a été supprimée.',
        icon: 'success',
        timer: 2000,
...
function Chnage_status(id: any) {
  Swal.fire({
    title: 'Activation',
    text: 'Voulez-vous activer cette année budgétaire ?',
    icon: 'question',
    showCancelButton: true,
    ...
    if (res.isConfirmed) {
      exerciceStore.activer_anner(id)
      Swal.fire({
        title: 'Activé!',
        text: 'L\'année budgétaire a été activée.',
        icon: 'success',
        timer: 2000,
        showConfirmButton: false
      })
...
function desactiver_anner(id: any) {
  Swal.fire({
    title: 'Désactivation',
    text: 'Voulez-vous désactiver cette année budgétaire ?',
    icon: 'warning',
    showCancelButton: true,
    ...
    if (res.isConfirmed) {
      exerciceStore.desactiver_anner(id)
      Swal.fire({
        title: 'Désactivé!',
        text: 'L\'année budgétaire a été désactivée.',
        icon: 'success',
        timer: 2000,
        showConfirmButton: false
      })
...
const showEditModal = (id: number) => {
  const d_data = exerciceStore.getterExerciceBudgetaire.find((item) => item.id === id)

  if (d_data) {
    formData.annee = d_data.annee
...
const verif_annee = (an: number) => {
  if (!an) return false
  const d_data = exerciceStore.getterExerciceBudgetaire.find((item) => item.annee == an)
  return !!d_data
}
...
const recupannverif = (an: number, date: string) => {
  if (date && an && an != 0) {
    return new Date(date).getFullYear() == an ? 1 : 0
  }
  return 1
}
...
function ajouterExercice() {
  $v.value.$touch()
  if (!$v.value.$invalid && !verif_annee(form.annee) && compareDate()) {
    try {
      isLoading.value = true
      exerciceStore.ajouterExerciceBudgetaire(form).then(() => {
...
function modifierExercice() {
  ...
  if (!$v_edit.value.$invalid && compareDateEdit()) {
    try {
      isLoading.value = true
      exerciceStore.modifierExerciceBudgetaire(formData).then(() => {
        isLoading.value = false
        closeModal()
      })
    } catch (error) {
